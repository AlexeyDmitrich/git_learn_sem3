...
для максимально полного, но при этом краткого просмотра всего дерева репозитория можно воспользоваться дополнительным параметром --oneline:
...
При слиянии методом merge, часто возникают конфликты. Git предупредит об этом. Конфликты редактируются в конечном файле вручную, либо графически, с помощью IDE. Если конфликтов не возникнет, git молча сольёт ветки, используя какую-то из своих стратегий.
...
*new
    ...
    далее будет предложен выбор вариантов его заполнения:
        создать репозиторий прямо на сайте или загрузить файлы с ПК
        создать новый репозиторий на ПК и загрузить его на сайт
        загрузить в облако готовый репозиторий
...
Вежливая работа с чужим проектом

Для совместной работы над проектом, на сайте GitHub нажимаем кнопку

fork

Это скопирует удалённый репозиторий автора в наше облако. Уже эту копию мы клонируем к себе на устройство с помощью описанной выше процедуры

git clone

В получившемся локальном репозитории следует создать новую ветку и все работы проводить, отталкиваясь от неё.
Внесение предложений автору проекта

Для того, чтобы предложить автору свои правки, следует закоммитить свои изменения и отправить их в свое облако командой

git push

* команда может не отработать, в таком случае git сам предложит правильное решение

В нашем профиле на GitHub появится кнопка pull request. При её нажатии, откроется форма для отправки предложения автору.
